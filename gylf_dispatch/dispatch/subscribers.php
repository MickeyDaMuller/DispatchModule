							...
							<!-- Post -->
								<article class="is-post is-post-excerpt">
									<header>
										...
										<span class="byline">Subscribers </span>
									</header>
									<div class="info">
...
function get_newsletters()
	...
	return $recs;
		...
		if($newsletterId == "")
		echo "Kindly select a subscription list: <br>";
		else
		{	
			$statement = "SELECT COUNT(email) as `num` FROM contacts left join subscriptions using(list_Id) where email LIKE '%$email%' AND newsletterId LIKE '%$newsletterId%' ";
	
	 
	 	 	$sql = "SELECT * FROM contacts left join subscriptions using(list_Id) where email LIKE '%$email%' AND newsletterId LIKE '%$newsletterId%' group by email order by subscriptions.timestamp desc LIMIT {$startpoint} , {$limit}";
	 // echo $sql;
...
?>
									...
									<a href="import_subscribers.php">Import contact</a>		
								</article>
